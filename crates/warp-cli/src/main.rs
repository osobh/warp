...
use warp_cli::{AliasAction, Cli, Commands, LegalHoldAction, RetentionAction, StreamAction};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Listen { port, bind } => warp_cli::commands::listen::execute(&bind, port).await,
        Commands::Plan {
            source,
            destination,
        } => warp_cli::commands::plan::execute(&source, &destination).await,
        Commands::Probe { server } => warp_cli::commands::probe::execute(&server).await,
        Commands::Info => warp_cli::commands::info::execute().await,
        Commands::Resume { session } => warp_cli::commands::resume::execute(&session).await,
        ...
        Commands::Stream { action } => match action {
            StreamAction::Encrypt {
                password,
                chunk_size,
                no_gpu,
                progress,
            } => {
                warp_cli::commands::stream::encrypt(
                    password.as_deref(),
                    chunk_size,
                    no_gpu,
                    progress,
        ...
        Commands::Remove {
            ...
            warp_cli::commands::storage::remove(
                &path,
                recursive,
                force,
                bypass_governance,
        ...
        Commands::Alias { action } => match action {
            ...
            AliasAction::List => warp_cli::commands::storage::alias_list().await,
        },
    }
}
