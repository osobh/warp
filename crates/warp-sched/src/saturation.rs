...
impl SaturationDetector {
    ...
    pub fn is_saturated(&self, metrics: &DynamicEdgeMetrics) -> bool {
        ...
        if metrics.rtt_samples.len() >= self.min_samples {
            if metrics.rtt_trend == RttTrend::Increasing {
                return true;
            }
        }
    ...
    pub fn with_rtt_threshold(mut self, threshold: f32) -> Self {
    ...
    pub fn with_penalty_scale(mut self, scale: f32) -> Self {
