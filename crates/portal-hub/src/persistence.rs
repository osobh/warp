...
impl PersistentStorage {
    ...
    pub fn store_chunk(&self, content_id: ContentId, encrypted_data: Vec<u8>) {
        ...
        if let Err(e) = self.chunks.insert(&content_id, encrypted_data.as_slice()) {
    ...
    pub fn store_manifest(&self, portal_id: PortalId, encrypted_manifest: Vec<u8>) {
        if let Err(e) = self
            .manifests
            .insert(portal_id.as_bytes(), encrypted_manifest.as_slice())
        {
...
impl HybridStorage {
    ...
    /// Store a chunk
    pub fn store_chunk(&self, content_id: ContentId, data: Vec<u8>) {
        match self {
